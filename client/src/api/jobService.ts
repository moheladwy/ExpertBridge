...
export const acceptOffer = async (offerId: string): Promise<JobResponse> => {
  ...
  const response = await fetch(`${API_BASE_URL}/jobs/offers/${offerId}/accept`, {
    method: 'PATCH',
    headers: {
...
export const acceptApplication = async (applicationId: string): Promise<JobResponse> => {
  const token = await getAuthToken();
  
  const response = await fetch(`${API_BASE_URL}/jobs/applications/${applicationId}/accept`, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`,
