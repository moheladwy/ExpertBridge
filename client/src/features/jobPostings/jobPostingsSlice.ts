...
export const jobPostingsApiSlice = apiSlice.injectEndpoints({
	endpoints: (builder) => ({
					...
					try {
						...
						const getPostsPatchResult = lifecycleApi.dispatch(
							jobPostingsApiSlice.util.updateQueryData(
								...
								(draft) => {
									console.log(draft);
									draft.pages[0].jobPostings.push(
										createdPosting
									);
								}
							)
		...
		applyToJobPosting: builder.mutation<
			JobApplicationResponse,
			ApplyToJobPostingRequest
		>({
			query: (request) => ({
...
export const selectJobPostingById = createSelector(
	...
	(postings, postingId) =>
		postings.find((posting) => posting.id === postingId)
);
