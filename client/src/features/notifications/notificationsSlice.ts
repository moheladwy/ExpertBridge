      ...
      onCacheEntryAdded: async (currentUserId, lifecycleApi) => {
        ...
        try {
          ...
          connection.on('ReceiveNotification', (notification: NotificationResponse) => {
            ...
            lifecycleApi.dispatch(newNotificationsReceived(notification));
          });

        } catch (error) {
          console.error('Connection to Notifications Hub failed: ', error);
        }
      },
    }),
      ...
      onQueryStarted: async (userId, lifecycleApi) => {
        const patchResult = lifecycleApi.dispatch(
          notificationsApiSlice.util.updateQueryData(
            ...
            userId,
            (draft) => {
              draft.forEach(notification => notification.isRead = true);
            }
          ),
        );
        ...
        try {
          ...
          console.error('An error occurred while reading notifications', error);
        }
      },
    }),
