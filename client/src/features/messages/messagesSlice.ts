		...
		getMessagesByChatId: builder.query<MessageResponse[], string>({
			...
			onCacheEntryAdded: async (chatId, lifecycleApi) => {
				...
				try {
					...
					console.log(
						"Connected to Notifications Hub for messaging."
					);

					connection.on(
						"ReceiveMessage",
						(message: MessageResponse & { receiverId: string }) => {
					...
					console.error(
						"Connection to SignalR hub failed (messages):",
						error
					);
				}
...
export const { useGetMessagesByChatIdQuery, useSendMessageMutation } =
	messagingApiSlice;
