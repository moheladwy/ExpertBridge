  ...
  const addTag = (tagName: string) => {
    ...
    const matchedTag = tags.find(
      (tag) =>
        ...
        (tag.arabicName && tag.arabicName.toLowerCase() === normalizedTagName),
    );
            ...
            <div className="flex flex-wrap gap-2">
              {selectedTags.length === 0 ? (
                <p className="text-sm text-gray-500 dark:text-gray-400 italic transition-colors duration-200">
                  No keywords selected yet. Select at least 5 to continue.
                </p>
              ) : (
                      ...
                      ((tag.englishName &&
                        ...
                        (tag.arabicName &&
                          tag.arabicName
                            .toLowerCase()
                            .includes(normalizedInput))) &&
                      !selectedTags.includes(tag.englishName.toLowerCase())
                    );
                  })
                  .slice(0, 5)
                  .map((tag) => (
                    <button
                      key={tag.tagId}
                      onClick={() => addTag(tag.englishName)}
