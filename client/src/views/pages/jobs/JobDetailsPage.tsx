...
export const JobDetailsPage: React.FC = () => {
  ...
  const { data: job, isLoading, error } = useGetJobByIdQuery(jobId!, { skip: !jobId });
  ...
  if (!authUser || !appUser) {
    return (
      <div className="text-center p-8">
        <p className="text-gray-600 dark:text-gray-400">Please log in to view job details.</p>
      </div>
    );
  }
  ...
  const handleConfirmAction = () => {
    ...
    toast.success(isWorker ? 'Job completion confirmed!' : 'Payment confirmation sent!');
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900 dark:text-white">${job.actualCost}</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900 dark:text-white">{job.area}</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900 dark:text-white">{formatDate(job.startedAt)}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                {isWorker ? 'Confirm Job Completion' : 'Confirm Payment'}
              </h3>
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                💡 <strong>Tip:</strong> Both parties should confirm their respective actions -
                the worker confirms job completion and the hirer confirms payment received.
              </p>
            </div>
            <Button
              onClick={handleConfirmAction}
              disabled={isWorker ? job.isCompleted : job.isPaid}
              className={`px-6 py-3 ${(isWorker && job.isCompleted) || (!isWorker && job.isPaid)
                ? 'bg-gray-100 text-gray-400 cursor-not-allowed dark:bg-gray-600 dark:text-gray-400'
                : 'bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600 text-white'
                }`}
            >
              {isWorker
                ? (job.isCompleted ? 'Job Confirmed' : 'Confirm Job is Done')
                : (job.isPaid ? 'Payment Confirmed' : 'Confirm Receiving Money')
              }
            </Button>
          </div>
        </div>
