...
import { Calendar, MapPin, DollarSign, MessageCircle, CheckCircle, Clock, User } from 'lucide-react';
  ...
  if (!authUser || !appUser) {
    return (
      <div className="text-center p-8">
        <p className="text-gray-600 dark:text-gray-400">Please log in to view your jobs.</p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((i) => (
            <div key={i} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border dark:border-gray-700 p-6">
    ...
    <div className="w-full flex justify-center">
      <div className="mt-5 w-3/5 max-xl:w-3/5 max-lg:w-4/5 max-sm:w-full bg-white dark:bg-gray-800 rounded-lg shadow-md border dark:border-gray-700 p-6">
        <div className="mb-8">
          ...
          <p className="text-gray-600 dark:text-gray-400">Manage your active and completed jobs</p>
        </div>

        {/* Tab Navigation using your Tabs component */}
        <Tabs defaultValue="active" onValueChange={(value) => setSelectedTab(value as 'active' | 'completed')}>
          ...
          <TabsContent value="active" className="space-y-4">
            {activeJobs.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {activeJobs.map((job) => (
                  <JobCard key={job.id} job={job} appUser={appUser} onViewDetails={() => navigate(`/my-jobs/${job.id}`)} />
                ))}
              </div>
            ) : (
