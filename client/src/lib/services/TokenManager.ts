...
import { authStateManager, getAuthUser } from "./AuthStateManager";
...
class TokenManagerClass {
	...
	constructor() {
		this.initializeWithCentralizedAuth();
	}

	/**
	 * Initialize with centralized auth state manager
	 * This eliminates duplicate onAuthStateChanged listeners
	 */
	private initializeWithCentralizedAuth(): void {
		// Subscribe to user changes from centralized auth manager
		this.authStateUnsubscribe = authStateManager.subscribeToUserChanges(
			(user) => {
				if (!user) {
					this.clearCache();
		...
		console.log(
			"[TokenManager] Using centralized auth state - no duplicate listeners!",
		);
	}
	...
	getStatus(): TokenStatus {
		const user = getAuthUser(); // Use centralized auth state
	...
	private async performRefresh(user: User, attempt = 1): Promise<string> {
		try {
			console.debug(
				`[TokenManager] Refreshing token (attempt ${attempt})`,
			);
			...
			console.error(
				`[TokenManager] Token refresh failed (attempt ${attempt}):`,
				error,
			);
			...
			this.clearCache();
			throw new Error(
				`Failed to refresh token after ${this.MAX_RETRY_ATTEMPTS} attempts`,
			);
		}
	}
	...
	async ensureFreshToken(): Promise<void> {
		...
		if (
			!status.isValid ||
			(status.expiresIn && status.expiresIn < 300000)
		) {
	...
	dispose(): void {
		if (this.authStateUnsubscribe) {
			// Unsubscribe from centralized auth manager
