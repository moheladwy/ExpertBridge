...
public sealed class PostQueriesTests : IDisposable
{
    ...
    public async Task FullyPopulatedPostQuery_ShouldIncludeMedias()
    {
        ...
        var media = TestDataBuilder.CreatePostMedia(post.Id, type: "image/jpeg", key: "test-image.jpg");

        _context.Profiles.Add(authorProfile);
        _context.Posts.Add(post);
        _context.PostMedias.Add((PostMedia)media);
        await _context.SaveChangesAsync();
    ...
    public async Task FullyPopulatedPostQuery_ShouldIncludePostTagsWithTags()
    {
        ...
        var tag = TestDataBuilder.CreateTag("Technology", "تكنولوجيا", "tag-1");
        var postTag = TestDataBuilder.CreatePostTag(post.Id, tag.Id);
    ...
    public async Task SelectPostResponseFromFullPost_ShouldSetIsDownvotedTrueWhenUserDownvoted()
    {
        ...
        var downvote = TestDataBuilder.CreatePostVote(post.Id, voterProfile.Id, false);
    ...
    public async Task SelectPostResponseFromFullPost_ShouldSetBothVoteFlagsFalseWhenUserNotVoted()
    {
        ...
        var result = await _context.Posts
            .FullyPopulatedPostQuery()
            .SelectPostResponseFromFullPost(nonVoterProfile.Id)
            .FirstAsync();
    ...
    public async Task SelectPostResponseFromFullPost_ShouldMapTagsCorrectly()
    {
        ...
        var tag1 = TestDataBuilder.CreateTag("Technology", "تكنولوجيا", "tag-1");
        var tag2 = TestDataBuilder.CreateTag("Programming", "برمجة", "tag-2");
        var postTag1 = TestDataBuilder.CreatePostTag(post.Id, tag1.Id);
        var postTag2 = TestDataBuilder.CreatePostTag(post.Id, tag2.Id);
    ...
    public async Task SelectPostResponseFromFullPost_ShouldMapMediasCorrectly()
    {
        ...
        var media1 = TestDataBuilder.CreatePostMedia(post.Id, type: "image/jpeg", key: "image1.jpg");
        var media2 = TestDataBuilder.CreatePostMedia(post.Id, type: "video/mp4", key: "video1.mp4");
