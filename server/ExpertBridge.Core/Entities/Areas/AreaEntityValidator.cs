...
public class AreaEntityValidator : AbstractValidator<Area>
{
    ...
    public AreaEntityValidator()
    {
        RuleFor(area => area.Id)
            ...
            .WithMessage($"Id is longer than {GlobalEntitiesConstraints.MaxIdLength} characters");

        RuleFor(area => area.ProfileId)
            .NotNull().WithMessage("ProfileId is required")
            .NotEmpty().WithMessage("ProfileId is required")
            .MaximumLength(GlobalEntitiesConstraints.MaxIdLength)
            .WithMessage($"ProfileId is longer than {GlobalEntitiesConstraints.MaxIdLength} characters");
